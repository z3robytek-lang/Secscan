...
from secscan.validators import NginxValidator, DockerValidator
...
def scan(directory, format):
    ...
    click.echo(f"Found {len(nginx_files)} Nginx, {len(docker_files)} Dockerfile, {len(compose_files)} docker-compose files\n")
...
def _display_results(result: dict, format: str, output_path: str = None, platform: str = ""):
    ...
    if result['issues']:
        ...
        for issue in result['issues']:
            severity_color = Fore.RED if issue.get('severity') == 'critical' else Fore.YELLOW
            click.echo(f"\n{severity_color}[{issue.get('severity', 'ISSUE').upper()}] {issue['check']}")
...
def _display_scan_summary(results: list):
    ...
    click.echo(tabulate(
        summary_data,
        headers=['File', 'Critical', 'High', 'Warnings', 'Passed'],
        tablefmt='grid'
    ))
